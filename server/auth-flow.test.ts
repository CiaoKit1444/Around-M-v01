...
const EXPRESS_BASE = process.env.EXPRESS_BASE || "http://localhost:3000";

describe("Auth flow — Express-native login endpoint", () => {
...
describe("Auth flow — Express login proxy", () => {
  it("should handle /api/v1/auth/login", async () => {
    try {
      const res = await axios.post(
        `${EXPRESS_BASE}/api/v1/auth/login`,
  ...
  it("should handle /api/v1/auth/me", async () => {
    try {
      const res = await axios.get(`${EXPRESS_BASE}/api/v1/auth/me`, {
        timeout: 5000,
        validateStatus: () => true,
...
describe("Front Office — request status update endpoint", () => {
  ...
  it("should handle status update through Express", async () => {
    try {
      const res = await axios.put(
        `${EXPRESS_BASE}/api/v1/front-office/requests/fake-id/status`,
        { status: "CONFIRMED" },
...
describe("QR Code — detail endpoint", () => {
  ...
  it("should handle QR detail through Express", async () => {
    try {
      const res = await axios.get(
        `${EXPRESS_BASE}/api/v1/properties/pr-001/qr/test-qr-id`,
        { timeout: 5000, validateStatus: () => true }
  ...
  it("should handle QR lifecycle actions through Express", async () => {
    try {
      const res = await axios.post(
